 ...
 */
object FlmWriter {
    /** Current FL Studio Mobile EVN2 timing uses 128 position units per beat. */
    ...
    fun write(project: FlpProject, projectName: String): ByteArray {
        ...
        repeat(outputCount) { index ->
            output += buildGeneratorRack(racks[1], index)
        }

        output += channels.first()
        repeat(outputCount) { index ->
            val sourceChannel = project.channels.getOrNull(index)
            val name = sourceChannel?.name?.takeIf { it.isNotBlank() }
                ?: "Canal ${index + 1}"
    ...
    private fun convertTicks(ticks: Long, sourcePpq: Int): Int {
        val scaled = ticks.toDouble() * FLM_TICKS_PER_BEAT /
            sourcePpq.coerceAtLeast(1)
